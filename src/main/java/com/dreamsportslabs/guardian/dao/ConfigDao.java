...
import static com.dreamsportslabs.guardian.dao.query.ConfigQuery.COOKIE_CONFIG;
...
public class ConfigDao {
  ...
  public Single<TenantConfig> getTenantConfig(String tenantId) {
    ...
    List<Completable> configSources =
        List.of(
            ...
            appendCookieConfig(tenantId, builder),
  ...
  private Completable appendCookieConfig(
      String tenantId, TenantConfig.TenantConfigBuilder builder) {
    return getConfigFromDb(tenantId, CookieConfig.class, COOKIE_CONFIG)
        .map(builder::cookieConfig)
        .ignoreElement();
  }
  ...
  private <T> Single<T> getConfigFromDb(String tenantId, Class<T> configType, String query) {
    return mysqlClient
        ...
        .doOnSuccess(o -> log.info(o.toString()));
  }
}
