...
public class ContactExtractionService {
    ...
    /**
     * Extract contact information from sign in request
     * For sign in, the username could be email or phone number
     */
    ...
    /**
     * Extract contact information from sign up request
     * For sign up, the username could be email or phone number
     */
    ...
    /**
     * Extract contact information from passwordless complete request
     * This requires looking up the state to get the original contact information
     */
    public Single<List<String>> extractContactsFromPasswordlessComplete(V1PasswordlessCompleteRequestDto requestDto, String tenantId) {
        if (StringUtils.isBlank(requestDto.getState())) {
            return Single.just(List.of());
        }
        
        return passwordlessDao.getPasswordlessModel(requestDto.getState(), tenantId)
                .map(model -> {
